...
--   MOD WOOD
-- ==========================================
local MOD_WOOD_POS1 = Vector3.new(-1413.0, 433.4, 1261.0)  -- staging position
local MOD_WOOD_POS2 = Vector3.new(-69.3,   42.2, -2214.3)  -- cut position

local _modWoodActive   = false
local _modWoodPhase    = 0   -- 1 = awaiting sawmill, 2 = awaiting log
local _selectedSawmill = nil
local _sawmillOutline  = nil
local _modWoodConn     = nil
local _modWoodButton   = nil

local function ClearSawmillOutline()
    if _sawmillOutline then _sawmillOutline:Destroy(); _sawmillOutline = nil end
end

local function ApplySawmillOutline(model)
    ClearSawmillOutline()
    _sawmillOutline                     = Instance.new("SelectionBox")
    _sawmillOutline.Adornee             = model
    _sawmillOutline.Color3              = Color3.fromRGB(255, 200, 0)
    _sawmillOutline.LineThickness       = 0.08
    _sawmillOutline.SurfaceColor3       = Color3.fromRGB(255, 200, 0)
    _sawmillOutline.SurfaceTransparency = 0.65
    _sawmillOutline.Parent              = Workspace
end

-- Walk up the part hierarchy looking for a Model named like a sawmill.
local function FindSawmillFromPart(part)
    local current = part
    while current and current ~= Workspace do
        if current:IsA("Model") and current.Name:lower():find("sawmill") then
            return current
        end
        current = current.Parent
    end
...
-- Walk up looking for a direct child of Workspace.LogModels.
local function FindLogModelFromPart(part)
    local logModels = Workspace:FindFirstChild("LogModels")
    if not logModels then return nil end
    local current = part
    while current and current ~= Workspace do
        if current:IsA("Model") and current.Parent == logModels then
            return current
        end
        current = current.Parent
    end
...
-- Returns the WoodSection with the smallest volume (the tip/branch).
local function GetSmallestSection(logModel)
    local pick, pickVol = nil, math.huge
    for _, part in ipairs(logModel:GetDescendants()) do
        if part:IsA("BasePart") and part.Name == "WoodSection" then
            local vol = part.Size.X * part.Size.Y * part.Size.Z
            if vol < pickVol then pickVol = vol; pick = part end
        end
    end
    return pick
end

-- Returns the WoodSection sitting lowest in the model (the base to cut).
local function GetLowestSection(logModel)
    local pick, pickY = nil, math.huge
    for _, part in ipairs(logModel:GetDescendants()) do
        if part:IsA("BasePart") and (part.Name == "WoodSection" or part.Name == "InnerWood") then
            if part.Position.Y < pickY then pickY = part.Position.Y; pick = part end
        end
    end
    return pick
end

-- Returns a CFrame at the centre of the sawmill's bounding box.
local function GetSawmillCenter(sawmill)
    if sawmill.PrimaryPart then return sawmill.PrimaryPart.CFrame end
    local sum, count = Vector3.new(), 0
    for _, part in ipairs(sawmill:GetDescendants()) do
        if part:IsA("BasePart") then sum += part.Position; count += 1 end
    end
    return count > 0 and CFrame.new(sum / count) or CFrame.new()
end

local function StopModWood()
    _modWoodActive   = false
    _modWoodPhase    = 0
    _selectedSawmill = nil
    ClearSawmillOutline()
    if _modWoodConn then _modWoodConn:Disconnect(); _modWoodConn = nil end
    if type(_modWoodButton) == "table" and _modWoodButton.SetText then
        _modWoodButton:SetText("Start")
    end
end

local function RunModWood(logModel, LOT)
    local sawmill = _selectedSawmill
    StopModWood()

    -- Resolve a reference part for LOT teleports (use PrimaryPart or first BasePart).
    local refPart = logModel.PrimaryPart
    if not refPart then
        for _, v in ipairs(logModel:GetDescendants()) do
            if v:IsA("BasePart") then refPart = v; break end
        end
    end
    if not refPart then warn("[ModWood] LogModel has no BasePart."); return end

    -- 1. TP the log to the staging position.
    while LOT.IsBusy() do task.wait(0.1) end
    LOT.TeleportObjectTo(refPart, CFrame.new(MOD_WOOD_POS1))
    repeat task.wait(0.1) until not LOT.IsBusy()
    print("[ModWood] Log at staging position.")

    -- 2. Unanchor the smallest section (the branch/tip we want to keep).
    local smallSection = GetSmallestSection(logModel)
    if not smallSection then warn("[ModWood] No WoodSection found to unanchor."); return end
    smallSection.Anchored = false
    print("[ModWood] Unanchored section:", smallSection.Name, "size:", smallSection.Size)

    -- 3. TP the log to the cut position.
    while LOT.IsBusy() do task.wait(0.1) end
    LOT.TeleportObjectTo(refPart, CFrame.new(MOD_WOOD_POS2))
    repeat task.wait(0.1) until not LOT.IsBusy()
    print("[ModWood] Log at cut position.")

    -- 4. Grab a tool and fire the remote on the lowest section to detach the tip.
    local tool, axeName = GetBackpackAxe()
    if not tool then warn("[ModWood] No tool in Backpack. Cannot cut."); return end

    local bottomSection = GetLowestSection(logModel)
    if not bottomSection then warn("[ModWood] No bottom section found."); return end

    local tcObj     = logModel:FindFirstChild("TreeClass")
    local treeClass = tcObj and tcObj.Value or "Generic"

    -- Fire a few sweeps — enough to sever the section, not destroy the whole log.
    for _ = 1, 3 do
        FireCutSection(bottomSection, tool, axeName, treeClass)
        task.wait(Settings.SweepDelay)
        if not bottomSection.Parent then break end
    end
    print("[ModWood] Cut complete.")

    -- 5. Give physics a moment to settle, then TP the freed piece into the sawmill.
    task.wait(0.6)

    if not smallSection.Parent then
        warn("[ModWood] Unanchored section is gone — it may have been destroyed by the cut.")
        return
    end

    local sawmillCF = GetSawmillCenter(sawmill)
    while LOT.IsBusy() do task.wait(0.1) end
    LOT.TeleportObjectTo(smallSection, sawmillCF)
    repeat task.wait(0.1) until not LOT.IsBusy()
    print("[ModWood] Section delivered to sawmill. Done!")
end

local function StartModWood(LOT)
    if _modWoodActive then StopModWood(); return end

    _modWoodActive = true
    _modWoodPhase  = 1
    print("[ModWood] Phase 1 — click a Sawmill to select it.")

    local mouse = player:GetMouse()

    _modWoodConn = mouse.Button1Down:Connect(function()
        if not _modWoodActive then return end

        if _modWoodPhase == 1 then
            local sawmill = FindSawmillFromPart(mouse.Target)
            if not sawmill then print("[ModWood] Not a sawmill — try again."); return end
            _selectedSawmill = sawmill
            ApplySawmillOutline(sawmill)
            _modWoodPhase = 2
            print("[ModWood] Sawmill selected! Phase 2 — click a loose log/tree.")

        elseif _modWoodPhase == 2 then
            local logModel = FindLogModelFromPart(mouse.Target)
            if not logModel then print("[ModWood] Not a loose log — try again."); return end
            print("[ModWood] Log selected — running sequence...")
            task.spawn(RunModWood, logModel, LOT)
        end
    end)
end
...
function TreeModule.Init(Tab, LOT)
    ...
    Tab:CreateSection("Mod Wood")

    _modWoodButton = Tab:CreateAction("Mod Wood", "Start", function()
        StartModWood(LOT)
    end)
end
