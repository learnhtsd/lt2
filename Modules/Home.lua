...
function HomeModule.Init(Tab, Library)
    ...
    -- 1. Welcome InfoBox
    Tab:CreateInfoBox("Hello, " .. Username .. "!", "Thank you for using Dynxe. You are currently using an early version. Im alone and need time :)")
    Tab:CreateInfoBox("Information", "Version: v0.0.101\nStatus: Online\nLast Updated: April 2026")
    ...
    local function ServerHop(order)
        ...
        -- Queue the script to run in the next server
        if queue_on_teleport then
            queue_on_teleport(LoadScript)
        end

        Library:Notify("System", "Searching for " .. (order == "Asc" and "Smallest" or "Largest") .. " server...", 4)

        -- Fetch servers based on order (Asc = Smallest first, Desc = Largest first)
